...
@implementation UIImage (XYWebImage)

- (UIImage *)xy_imageByGrayscale {
    return [self xy_imageByBlurRadius:0 tintColor:nil tintMode:0 saturation:0 maskImage:nil];
}
...
- (UIImage *)xy_imageByBlurExtraLight {
    return [self xy_imageByBlurRadius:40 tintColor:[UIColor colorWithWhite:0.97 alpha:0.82] tintMode:kCGBlendModeNormal saturation:1.8 maskImage:nil];
}
...
- (UIImage *)xy_imageByBlurWithTint:(UIColor *)tintColor {
    ...
    return [self xy_imageByBlurRadius:20 tintColor:effectColor tintMode:kCGBlendModeNormal saturation:-1.0 maskImage:nil];
}
...
- (UIImage *)xy_imageByBlurRadius:(CGFloat)blurRadius
    ...
    if (hasNewFunc) {
        ...
        effectCGImage = vImageCreateCGImageFromBuffer(input, &format, &_xy_cleanupBuffer, NULL, kvImageNoAllocate, NULL);
...
static void _xy_cleanupBuffer(void *userData, void *buf_data) {
    free(buf_data);
}
